"""
PRECISION Ball Tracking - No Gimmicks, Just Works
- Detects ball EVERY frame (no skipping!)
- Very low confidence (finds every possible ball)
- Large detection area (never misses)
- Smart smoothing (precise but not jittery)
- Landscape output (original aspect ratio)
- Focus: NEVER LOSE THE BALL
"""
...
IMG_SZ = 1280              # High resolution for better detection
DETECT_EVERY = 1           # EVERY FRAME - never miss!
ROI_SIZE = 2000            # HUGE search area

# Smoothing - Balanced (not too slow, not too jittery)
SMOOTH_CAM = 0.08          # Camera smoothing
SMOOTH_ZOOM = 0.10         # Zoom smoothing

# Zoom - Conservative (not too much)
ZOOM_BASE = 1.8
ZOOM_MIN = 1.5
ZOOM_MAX = 2.2
...
print(f"🎯 PRECISION Ball Tracking")
print(f"📹 Input: {W}x{H} @ {FPS}fps")
print(f"📹 Output: {OUTPUT_WIDTH}x{OUTPUT_HEIGHT} (landscape)")
print(f"🔍 Detection: EVERY frame, Conf={YOLO_CONF}, ROI={ROI_SIZE}px")
print(f"✨ Smoothing: Camera={SMOOTH_CAM}, Zoom={SMOOTH_ZOOM}")
...
ball_history = []
velocity_history = []
detection_history = []  # Store detection confidences
...
while True:
    ...
    # -------------------------------------------------
    # Use large ROI or full frame if ball position is uncertain
    ...
    # If ROI would be too small or ball is lost, use full frame
    if (x2 - x1) < W * 0.9 and (y2 - y1) < H * 0.9:
    ...
    if boxes is not None and len(boxes) > 0:
        for b in boxes:
            ...
            # Proximity score (closer to last position = more likely)
            if len(ball_history) > 0:
                proximity_score = 1.0 / (1.0 + dist / 200.0)  # Decay with distance
            else:
                proximity_score = 1.0
    ...
    # -------------------------------------------------
    # TRACKER UPDATE (as backup between detections)
    # -------------------------------------------------
    if tracker is not None and not detected_this_frame:
        ...
        if ok:
            ...
            # Only use tracker if it's not too far from expected position
            if len(ball_history) > 0:
                dist = np.linalg.norm(np.array([cx, cy]) - ball_center)
                if dist < 300:  # Reasonable distance
                    target_center = np.array([cx, cy], dtype=np.float32)
                    tracker_count += 1
                else:
                    tracker = None  # Too far, probably wrong
            else:
                target_center = np.array([cx, cy], dtype=np.float32)
                tracker_count += 1
        else:
    ...
    # -------------------------------------------------
    # VELOCITY-BASED PREDICTION (if no detection or tracker)
    # -------------------------------------------------
    ...
    # -------------------------------------------------
    # UPDATE BALL POSITION (with temporal filtering)
    # -------------------------------------------------
    if target_center is not None:
        if len(ball_history) > 0:
            ...
            if movement_size < 5:
                # Very small movement - heavy filtering
                blend = 0.3
            elif movement_size < 20:
                # Small movement - moderate filtering
                blend = 0.5
            elif movement_size < 100:
                # Medium movement - light filtering
                blend = 0.7
            else:
                # Large movement (probably a shot) - minimal filtering
                blend = 0.85
            
            # Smooth jump detection
            if movement_size > 200:
                # Probably a detection error, blend more carefully
                blend = 0.4
    ...
    # If completely lost, stay at last known position
    ...
    # Resize to output resolution (landscape)
...
print(f"✅ Precision Ball Tracking complete in {elapsed:.1f}s")
