...
# ===========================================
# ⭐ PixelReview - iOS Release Build
# ===========================================
# Configured identically to restaurantadmin
# ✅ Builds Next.js web bundle
# ✅ Syncs Capacitor iOS native assets
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}⭐ PixelReview - iOS Release${NC}"
echo -e "${BLUE}========================================${NC}"
...
echo -e "${CYAN}📦 Step 1: Building Next.js Web App...${NC}"
npm run build

echo -e "${CYAN}🔄 Step 2: Syncing Capacitor iOS project...${NC}"
npx cap sync ios

if [ "$AUTO_UPLOAD" = true ]; then
    echo -e "${MAGENTA}🚀 Step 3: Fastlane building & uploading to TestFlight...${NC}"
    bundle exec fastlane ios upload_app
elif [ "$MANUAL_ONLY" = true ]; then
    echo -e "${MAGENTA}🔨 Step 3: Fastlane building IPA (no upload)...${NC}"
