"""
FAST PRECISION Ball Tracking with Perspective Correction
...
# ----------------- CONFIG -----------------
# OPTIMIZED FOR ACCURACY + SPEED BALANCE
YOLO_CONF = 0.12           # Good detection threshold
IMG_SZ = 960               # Good balance (1.7x faster than 1280)
DETECT_EVERY = 3           # Every 3rd frame (balance speed + accuracy)
...
def calculate_perspective_zoom(ball_y, ball_speed):
    ...
    - Far side (top, y < 0.3*H): Ball is FAR, zoom IN more for detail
    ...
    if y_norm > NEAR_SIDE_THRESHOLD:
        ...
        # Zoom IN more to track detail
        distance_into_far = (FAR_SIDE_THRESHOLD - y_norm) / FAR_SIDE_THRESHOLD
        zoom_adjust = 0.3 * distance_into_far  # Zoom in up to +0.3
    else:
        ...
        zoom_adjust = 0.3 - (transition * 0.7)  # Smooth from +0.3 to -0.4
...
print(f"⚡ FAST PRECISION Ball Tracking (5x Faster!)")
...
print(f"✨ Optimizations: IMG_SZ={IMG_SZ} (1.7x faster), INTER_LINEAR (1.1x faster)")
...
while True:
    ...
    # OPTIMIZED: Use INTER_LINEAR (1.1x faster than LANCZOS4, still good quality)
...
print(f"✅ Fast Precision Ball Tracking complete in {elapsed:.1f}s")
...
print(f"⚡ Speed optimizations: DETECT_EVERY={DETECT_EVERY}, IMG_SZ={IMG_SZ}, ROI={ROI_SIZE}, INTER_LINEAR")
