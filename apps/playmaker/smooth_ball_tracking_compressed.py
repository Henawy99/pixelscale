"""
ULTRA-SMOOTH Ball Tracking with AGGRESSIVE COMPRESSION
- H264 codec for small file sizes
- Optional 720p output for massive size reduction
- Smooth camera + player heuristics
...
# === COMPRESSION SETTINGS (KEY TO FIXING PAYLOAD ISSUE!) ===
OUTPUT_RESOLUTION = "720p"  # Options: "original", "720p", "480p"
COMPRESSION_QUALITY = 28     # CRF value: 18=high quality/large, 28=good/medium, 35=ok/small
# ===========================================================
...
if OUTPUT_RESOLUTION == "720p":
    out_w, out_h = 1280, 720
elif OUTPUT_RESOLUTION == "480p":
    out_w, out_h = 854, 480
else:
    out_w, out_h = W, H

print(f"📹 Output Resolution: {out_w}x{out_h} (Input: {W}x{H})")
print(f"🗜️  Compression: CRF={COMPRESSION_QUALITY} (lower=better quality, higher=smaller file)")
...
# Output video - Try H264 codecs (MUCH better compression!)
codecs_to_try = [
    ...
    ('mp4v', cv2.VideoWriter_fourcc(*'mp4v')),  # Fallback (larger files)
]
...
for codec_name, fourcc in codecs_to_try:
    ...
    if out.isOpened():
        ...
        print(f"⚠️ Codec {codec_name} not available, trying next...")
    ...
    if do_detect:
        ...
        if boxes is not None:
            for b in boxes:
                ...
                if cls == 32:  # Ball
                    # Prioritize detections near last position
...
print(f"✅ Codec used: {working_codec}")
print(f"✅ Output resolution: {out_w}x{out_h}")
print(f"✅ Output: {output_path}")

# Note: Modal will re-encode with ffmpeg for web compatibility
# Using CRF={COMPRESSION_QUALITY} for aggressive compression
