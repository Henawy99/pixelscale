#!/bin/bash
# ═══════════════════════════════════════════════════════
# Animight — Android Release Build → Play Console
# ═══════════════════════════════════════════════════════
set -e
cd "$(dirname "$0")"
...
echo -e "${CYAN}🤖 Animight — Android Build + Play Console Upload${NC}"
echo ""

# Ensure Bundler deps are installed in android/
...
case "${1}" in
  --build-only)
    echo -e "${YELLOW}Building AAB only (no upload)...${NC}"
  ...
  --upload-only)
    ...
    ;;
  *)
    echo -e "${YELLOW}Building Flutter AAB + uploading to Play Console...${NC}"
